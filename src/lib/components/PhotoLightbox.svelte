<script lang="ts">
	import { gradient, imageUrl, type Photo } from '$lib/photos';
	import PhotoDetail from './PhotoDetail.svelte';

	// Keep LB_META_WIDTH in sync with the .lb-meta flex-basis CSS rule in
	// PhotoDetail.svelte. The panel's own max-width is a generous fallback (see
	// .lb-panel); the real cap is computed from the viewport below so
	// wide/landscape photos can use most of the available screen instead of
	// being boxed into a fixed width.
	...
	let { photos, selectedId = $bindable(null) }: Props = $props();
	...
	const current = $derived(photos.find((p) => p.id === selectedId) ?? null);
	const currentIndex = $derived(current ? photos.findIndex((p) => p.id === current.id) : -1);
	const prevIndex = $derived(
		currentIndex >= 0 ? (currentIndex - 1 + photos.length) % photos.length : -1
	);
	const nextIndex = $derived(currentIndex >= 0 ? (currentIndex + 1) % photos.length : -1);
	const prevPhoto = $derived(prevIndex >= 0 ? photos[prevIndex] : null);
	const nextPhoto = $derived(nextIndex >= 0 ? photos[nextIndex] : null);
	function counterFor(idx: number) {
		return `${String(idx + 1).padStart(2, '0')} / ${String(photos.length).padStart(2, '0')}`;
	}
	...
	function step(d: number) {
		...
		const n = (currentIndex + d + photos.length) % photos.length;
		selectedId = photos[n].id;
	}
